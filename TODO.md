...
- [ ] `RB-001` Initialize git repo, create `main` and `dev`, push to GitHub
- [ ] `RB-002` Create Vercel project and sync production env vars
- [ ] `RB-003` Verify magic link auth end-to-end on `localhost:3002`
- [ ] `RB-004` Align DB schema, TS types, mock data, and UI receipt statuses

## This Week

- [ ] `RB-005` Collect and organize 30 Taiwan receipt samples in `test-receipts/`
- [ ] `RB-006` Build `lib/ai/extract.ts` with strict schema and ROC date normalization
- [ ] `RB-007` Implement `POST /api/receipts/upload` with async extraction flow
